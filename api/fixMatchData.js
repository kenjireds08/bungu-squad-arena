...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Fix match status and player data
    await sheetsService.updateMatchStatus(matchId, 'approved', 'player_1753942268346_444ujdo4u');
    
    // Also ensure match data has correct player names
      ...
      await sheetsService.sheets.spreadsheets.values.update({
        ...
        requestBody: {
          values: [[
            ...
            'クリリン', // player1_name
            'player_1753943387023_8ndu3qxfh', // player2_id (correct ID for 天津飯)
            '天津飯', // player2_name
            'cardplus', // game_type
            'approved', // status
    ...
    return res.status(200).json({
      success: true,
      message: 'Match data fixed successfully'
    });
    
  } catch (error) {
    console.error('Error fixing match data:', error);
    return res.status(500).json({ 
      error: 'Failed to fix match data',
